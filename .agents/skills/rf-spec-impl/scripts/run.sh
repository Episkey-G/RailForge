...
if [[ -x "$RAILFORGE_BIN" ]]; then
  if [[ " $* " == *" --workspace "* ]]; then
    exec "$RAILFORGE_BIN" spec-impl "$@"
  fi

  exec "$RAILFORGE_BIN" spec-impl --workspace "$PWD" "$@"
fi
...
if [[ " $* " == *" --workspace "* ]]; then
  exec "$PYTHON_BIN" -m railforge spec-impl "$@"
fi

exec "$PYTHON_BIN" -m railforge spec-impl --workspace "$PWD" "$@"
