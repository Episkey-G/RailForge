...
if [[ -x "$RAILFORGE_BIN" ]]; then
  if [[ " $* " == *" --workspace "* ]]; then
    exec "$RAILFORGE_BIN" spec-research "$@"
  fi

  exec "$RAILFORGE_BIN" spec-research --workspace "$PWD" "$@"
fi
...
if [[ " $* " == *" --workspace "* ]]; then
  exec "$PYTHON_BIN" -m railforge spec-research "$@"
fi

exec "$PYTHON_BIN" -m railforge spec-research --workspace "$PWD" "$@"
