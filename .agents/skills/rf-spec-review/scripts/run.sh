...
if [[ -x "$RAILFORGE_BIN" ]]; then
  if [[ " $* " == *" --workspace "* ]]; then
    exec "$RAILFORGE_BIN" spec-review "$@"
  fi

  exec "$RAILFORGE_BIN" spec-review --workspace "$PWD" "$@"
fi
...
if [[ " $* " == *" --workspace "* ]]; then
  exec "$PYTHON_BIN" -m railforge spec-review "$@"
fi

exec "$PYTHON_BIN" -m railforge spec-review --workspace "$PWD" "$@"
