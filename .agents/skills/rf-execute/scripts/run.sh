...
if [[ -x "$RAILFORGE_BIN" ]]; then
  if [[ " $* " == *" --workspace "* ]]; then
    exec "$RAILFORGE_BIN" execute "$@"
  fi

  exec "$RAILFORGE_BIN" execute --workspace "$PWD" "$@"
fi
...
if [[ " $* " == *" --workspace "* ]]; then
  exec "$PYTHON_BIN" -m railforge execute "$@"
fi

exec "$PYTHON_BIN" -m railforge execute --workspace "$PWD" "$@"
