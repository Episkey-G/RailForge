...
if [[ -x "$RAILFORGE_BIN" ]]; then
  if [[ " $* " == *" --workspace "* ]]; then
    exec "$RAILFORGE_BIN" spec-plan "$@"
  fi

  exec "$RAILFORGE_BIN" spec-plan --workspace "$PWD" "$@"
fi
...
if [[ " $* " == *" --workspace "* ]]; then
  exec "$PYTHON_BIN" -m railforge spec-plan "$@"
fi

exec "$PYTHON_BIN" -m railforge spec-plan --workspace "$PWD" "$@"
