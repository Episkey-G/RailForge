...
class ClaudeCliSpecialistAdapter(SpecialistAdapter):
    ...
    def _build_prompt(self, kwargs: dict[str, Any]) -> str:
        return (
            ...
            "请始终参与双模型审查，并聚焦 correctness、maintainability、cross-cutting concerns 与 integration 风险，不直接越权扩 scope。\n"
            "你的主要视角偏后端与通用工程质量；与 Gemini 的前端/UX/integration 视角互补，但不要假设 Gemini 会替你发现 correctness 或 contract consistency 问题。\n\n"
            "请只输出 JSON，对象结构固定为：\n"
